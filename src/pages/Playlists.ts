...
} from "../services/userPlaylistsService";
            ...
            <li data-id="${playlist.id}" class="user-playlist-item">
                ${
                  playlist.images && playlist.images.length > 0
                    ? `<img src="${playlist.images[0].url}" alt="${playlist.name}" class="user-playlist-image" />`
                    : ""
                }
                <p class="user-playlist-name">${playlist.name}</p>
            </li>
        `
          )
          .join("")}
...
export async function renderUserPlaylistDetails(
  playlistId: string
): Promise<string> {
  const token = localStorage.getItem("accessToken");
  if (!token) {
  ...
  return `
    <div class="playlist-details">
            <div class="playlist-header">
                ...
                <h2 class="back-to-user-playlists">Mis Playlists / ${
                  playlist.name
                }</h2>
            </div>
